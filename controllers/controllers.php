...
class controllers extends models
{
    ...
    public function add_new_customer()
    {
        if (isset($_POST['add_new_customer'])) {
            ...
            $image_name = $_FILES['cus_image']['name'];
            $image_type = $_FILES['cus_image']['type'];

            $image_ext = pathinfo($image_name, PATHINFO_EXTENSION);

            $image_tmp_name = $_FILES['cus_image']['tmp_name'];

            $upload_dir = "./assets/img/client_image/";

            $upload_img = $upload_dir . $image_name;
            ...
            if ($image_ext == 'jpg' || $image_ext == 'jpeg' || $image_ext == 'png') {
                // echo '
                //     <script>
                //         danger_alert("Error !!", "Only jpg, jpeg and png file formats are allowed !!");
                //     </script>
                // ';
                ...
                if (move_uploaded_file($image_tmp_name, $upload_img)) {
                    // now continue the process
                    $add_cus = $this->insert("customers", " `cus_name`, `cus_email`, `cus_phone`, `cus_image_name` ", " '$cus_name', '$cus_email', '$cus_phone', '$image_name' ");
    ...
    public function edit_customer()
    {
        if (isset($_POST['update_customer'])) {
            ...
            if ($image_ext == 'jpg' || $image_ext == 'jpeg' || $image_ext == 'png') {
                ...
                if (move_uploaded_file($image_tmp_name, $upload_img)) {
                    $update_cus = $this->update_where("customers", " `cus_name` = '$cus_name', `cus_email` = '$cus_email', `cus_phone` = '$cus_phone', `cus_image_name` = '$image_name' ", " `cus_id` = '$get_cus_id' ");
    ...
    public function deposit($account_number, $cus_id, $amount, $description = '')
    {
        ...
        $inset_depo = $this->insert(
            "transactions",
            "`reference_id`, `account_number`, `customer_id`,  `transaction_type`, `amount`, `remarks`, `status`",
            "'$ref', '$account_number', '$cus_id', 'deposit', '$amount', '$description', 'success'"
        );
    ...
    public function withdraw($account_number, $cus_id, $amount, $description = '')
    {
        ...
        // Insert transaction
        $ref = uniqid("TXN_", true);
        $insert_withdraw = $this->insert(
            "transactions",
            "`reference_id`, `account_number`, `customer_id`, `transaction_type`, `amount`, `remarks`, `status`",
            "'$ref', '$account_number', '$cus_id', 'withdraw', '$amount', '$description', 'success'"
        );
    ...
    public function transfer($from_ac, $to_ac, $from_cus_id, $to_cus_id, $amount, $description = '')
    {
        ...
        $this->insert(
            "transactions",
            "`reference_id`, `account_number`, `customer_id`, `transaction_type`, `amount`, `remarks`, `status`",
            "'$ref', '$from_ac', '$from_cus_id', 'transfer_to_$to_ac', '$amount', '$description', 'success'"
        );
        ...
        $this->insert(
            "transactions",
            "`reference_id`, `account_number`, `customer_id`, `transaction_type`, `amount`, `remarks`, `status`",
            "'$ref', '$to_ac', '$to_cus_id', 'transfer_from_$from_ac', '$amount', '$description', 'success'"
        );
